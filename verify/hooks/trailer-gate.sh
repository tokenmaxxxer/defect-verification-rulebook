...
deny() { echo "verify: refused — $1" >&2; exit 2; }
...
VERIFY_CPD="${CLAUDE_PROJECT_DIR:-}" \
VERIFY_CWD="$(pwd -P 2>/dev/null || echo)" \
python3 <<'PY'
import json, os, posixpath, re, shlex, subprocess, sys

def deny(msg):
    sys.stderr.write("verify: refused — %s\n" % msg)
    sys.exit(2)
...
def _verify_fail_closed(_t, _v, _tb):
    try:
        sys.stderr.write("verify: refused — fail-closed: internal error (%s: %s)\n" % (_t.__name__, _v))
    except Exception:
        pass
    os._exit(2)
sys.excepthook = _verify_fail_closed

raw = os.environ.get("VERIFY_PAYLOAD", "")
try:
...
cpd = os.environ.get("VERIFY_CPD", "")
cwd = os.environ.get("VERIFY_CWD", "") or "."
...
if [ "$rc" -ne 0 ] && [ "$rc" -ne 2 ]; then
  echo "verify: refused — fail-closed: internal error (gate judge exited $rc)" >&2
  exit 2
fi
exit "$rc"
